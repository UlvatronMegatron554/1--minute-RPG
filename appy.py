...
div.stButton>button{{border:4px solid {C}!important;background:#000000!important;color:#ffffff!important;font-family:'Bebas Neue',sans-serif!important;font-size:20px!important;letter-spacing:3px!important;padding:22px 20px!important;border-radius:16px!important;animation:titan-pulse 2.5s infinite ease-in-out!important;width:100%;text-transform:uppercase;transition:transform 0.3s;margin-bottom:12px;}}
...
if view == "main":
    ...
    _shield_html = "<div style='font-size:11px;color:#00FF44;margin-top:4px'>🛡️ SHIELD ACTIVE</div>" if shield else ""
    _boost_html  = "<div style='font-size:11px;color:#FF8800;margin-top:4px'>🚀 3× BOOSTER ACTIVE</div>" if boost else ""
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    # ── Live prize wheel + SPIN IT — everything in one component ────────────────
    ...
    prize_types  = json.dumps([p["type"]  for p in SPINNER_PRIZES])
    prize_values = json.dumps([p["value"] for p in SPINNER_PRIZES])
    can_spin_js  = "true" if can_spin else "false"
    cooldown_label = f"LOCKED — {cooldown_secs_left//3600}h {(cooldown_secs_left%3600)//60}m REMAINING" if on_cooldown else "COMPLETE A MISSION TO EARN SPINS"
    components.html(f"""
<style>
...
#spinBtn:disabled{{opacity:0.35;cursor:not-allowed;background:#2a2a2a;color:#555;box-shadow:none;
...
document.getElementById('spinBtn').onclick=function(){{
  ...
  function anim(now){{
    var el=now-start, p=Math.min(el/dur,1), ease=1-Math.pow(1-p,4);
