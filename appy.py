...
if st.session_state.user_name is None:
    ...
    with col2:
        ...
        theme_input = st.text_input("World Theme:", placeholder="Leave blank for Instant Start")
            ...
            if name_input:
                if not theme_input.strip():
                    st.session_state.world_data = {
                        ...
                        "shield_lore": "A divine barrier of will.", "booster_lore": "Focus for the ultimate rank.",
...
st.markdown(f"<style>.main {{ background: #050505; color: white; }} div.stButton > button {{ border: 5px dashed {active_color} !important; animation: titan-glow 3s infinite; }}</style>", unsafe_allow_html=True)

# --- 4. SIDEBAR ---
with st.sidebar:
    st.title(f"⚡ {st.session_state.user_name.upper()}")
    st.write(f"**RANK:** {st.session_state.sub_tier} ({st.session_state.sub_multiplier}x XP)")
    ...
    if code == "TITAN10" and st.session_state.sub_tier != "Elite":
        ...
        st.success("👑 ELITE STATUS ACTIVATED!") # THE SUCCESS MESSAGE
...
st.header(st.session_state.user_theme.upper())
...
if st.session_state.view == 'shop':
    ...
    with col1:
        ...
        st.write(f"_{w.get('shield_lore', 'Defense.')}_")
    ...
    with col2:
        ...
        st.write(f"_{w.get('booster_lore', 'Speed.')}_")
