...
try:
    ...
    # TEMPERATURE 0.0 FOR FACTUAL ACCURACY (NO GUESSING)
...
if st.session_state.user_name is None:
    ...
    with col2:
        ...
        theme_input = st.text_input("World Theme (e.g. Formula 1, Minecraft, Naruto):")
        ...
        if st.button("🔥 AWAKEN"):
            if name_input:
                ...
                with st.spinner(f"EXTRACTING LORE FOR {t.upper()}..."):
                    try:
                        # THE LORE-STRIKER PROMPT: DEMANDING FACTS ONLY
                        prompt = (f"Identify the lore of '{t}'. "
                                 f"1. What is the actual currency used in '{t}'? "
                                 f"2. What is a famous defensive move/item in '{t}'? "
                                 f"3. What is a famous speed move/item in '{t}'? "
                                 f"RULES: Do NOT use the word '{t}' in the answer. Do NOT guess. "
                        ...
                        if match:
                            ...
                            # THE CLEANER: Scrub the theme name if the AI cheated
                        ...
                        st.error("Lore extraction failed. Try a more specific theme name.")
    st.stop()
